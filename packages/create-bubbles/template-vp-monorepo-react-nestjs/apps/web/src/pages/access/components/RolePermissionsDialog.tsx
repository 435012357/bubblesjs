...
export default function RolePermissionsDialog({
      ...
      /** 按页面组织权限树，把页面读取权限及其操作放入同一勾选组。 */ (page) => ({
        ...
        children: [
          ...
          /** 展示权限及废弃标记，仅允许调整既有授权或当前可授予的权限。 */ (permission) => ({
            key: permission.key,
            title: `${permission.kind === 'page' ? tr('访问页面') : tr(permission.title)}${permission.deprecated ? tr('（已废弃）') : ''}`,
  ...
  return (
    <Modal
      title={tr('{action}权限 · {name}', {
        action: readOnly ? tr('查看') : tr('配置'),
        name: record?.name ?? '',
      })}
      open={open}
      width={680}
      ...
      footer={
        readOnly ? (
          ...
          <Space>
            <Button onClick={hide}>{tr('取消')}</Button>
    ...
    >
      <Alert
        ...
        title={
          record?.builtin
            ? tr('内置角色由系统维护，不能人工编辑。')
            : tr('按钮授权自动包含页面访问权限；取消页面访问将同步取消页面下的操作。')
        }
        description={
          record?.builtin
            ? tr('新增功能自动授予对应内置管理员，仍受功能启停状态约束。')
            : tr(
      ...
      {preserved.length > 0 && (
        <Alert
          style={{ marginTop: 12 }}
          type="warning"
          title={tr('另有 {count} 项历史权限引用将保留，须经平台维护流程清理。', {
