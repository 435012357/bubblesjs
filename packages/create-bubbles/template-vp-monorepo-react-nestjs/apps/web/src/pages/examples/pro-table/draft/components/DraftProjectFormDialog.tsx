...
import { getPriorityOptions, getStatusOptions, owners, type ProjectFormValues } from '../../config'
...
export default function DraftProjectFormDialog({ ref, onSave }: DraftProjectFormDialogProps) {
  ...
  return (
    <ModalForm<ProjectFormValues>
      ...
      formRef={formRef}
      title={isPublished ? tr('编辑项目') : record ? tr('继续编辑草稿') : tr('新增项目')}
      open={open}
      width={600}
      grid
      rowProps={{ gutter: 16 }}
    ...
    >
      <ProFormText
        ...
        extra={!isPublished ? tr('信息未填完也可以保存草稿，提交时需补齐必填项。') : undefined}
      />
      <ProFormSelect
        name="owner"
        label={tr('负责人')}
