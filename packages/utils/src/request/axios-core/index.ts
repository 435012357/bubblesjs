...
export const getRequest = (option: getRequestOption) => {
  const statusMap = {
    success: option?.statusMap?.success ?? 200,
    unAuthorized: option?.statusMap?.unAuthorized ?? 401,
  }
  const codeMap = {
    success: option?.codeMap?.success ?? [200],
    unAuthorized: option?.codeMap?.unAuthorized ?? [],
  }
